...
export const ControlPanel = () => {
    ...
    useEffect(() => {
        const loadData = async () => {
            const dataFiltersPromise = Promise.all(Object.keys(mapFilters).map(async filterKey => ({
    ...
    return (
        <div data-testid="control-panel" className="control-panel-container">
            <h4>Map Controls</h4>
            <Button data-testid="button-reset" onClick={handleReset} variant="link">Reset</Button>
            <Button data-testid="button-clear-all" onClick={handleClearAll} variant="link">Clear all</Button>
            <hr/>
